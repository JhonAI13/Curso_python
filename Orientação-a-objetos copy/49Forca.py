    ...
    while jogando:
        ...
        if palpite in palavraSecreta:
            ...
            if len(letrasErradas) == len(FORCAIMG) - 1:
                ...
                print(f'Depois de  {str(len(letrasErradas))} letras erradas e {str(len(letrasAcertadas))}\npalpites corretos, a palavra era {palavraSecreta}')
...
def jogarNovamente():
    ...
    a resposta
    """
    input("Você quer jogar novamente? (sim ou não)\n").upper().startswith("S")
